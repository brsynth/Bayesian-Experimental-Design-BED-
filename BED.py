...
inference_variables = variable_media + [f"design_{name}" for name in variable_media] + ["y_cobra"]
...
df_reduced = df_reduced[inference_variables].dropna()
...
def apply_design_mask(prior_sample: dict, design: pd.Series) -> dict:
    ...
    for col in prior_sample:
        masked[col] = np.array(prior_sample[col]) * design[col]
    return masked
...
for i in range(n_designs):
    ...
    design_dict = {f"design_{col}": design[col] for col in variable_media}
    masked_prior.update(design_dict)
    log_prob = workflow.log_prob(masked_prior)
    print(f"Design {i+1}/{n_designs}: {np.mean(log_prob)}")
